...
CREATE TABLE IF NOT EXISTS `devshop`.`product_variations` (
  ...
  INDEX `fk_product_variations_produtos_idx` (`product_id` ASC),
...
CREATE TABLE IF NOT EXISTS `devshop`.`product_images` (
  ...
  CONSTRAINT `fk_product_images_product_variations1`
    ...
    REFERENCES `devshop`.`product_variations` (`idproduct_variations`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `devshop`.`categories_products` (
  ...
  INDEX `fk_categories_has_produtos_produtos1_idx` (`product_id` ASC),
  INDEX `fk_categories_has_produtos_categories1_idx` (`category_id` ASC),
  CONSTRAINT `fk_categories_has_produtos_categories1`
    FOREIGN KEY (`category_id`)
    REFERENCES `devshop`.`categories` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_categories_has_produtos_produtos1`
    FOREIGN KEY (`product_id`)
    REFERENCES `devshop`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
